...
class Setup:
    ...
    def startup_procedures(self, key=None):
        ...
        self.status_text = ttk.Label(self.auth_win, text=database.launch_text.upper())
        ...
        self.progress = ttk.Progressbar(self.auth_win, orient=HORIZONTAL, length=300, mode='determinate')
        self.progress.grid(column=0, row=1, padx=15, pady=10)
        # self.key_entry =  ttk.Entry(self.auth_win)
        # self.key_entry.grid(column=0, row=1, padx=15, pady=10)
        # self.key_entry.grid_remove()
        # self.okay_button = ttk.Button(self.auth_win, text='SUBMIT', command=self.submit_key)
